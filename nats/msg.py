...
class Msg:
    ...
    """
    def __init__(self, subject='', reply='', data=b'', headers=None, client=None):
    ...
    def metadata(self):
        ...
        if self._metadata is None:
            tokens = self._get_metadata_fields()
            ...
            if len(tokens) == Msg.Ack.V1TokenCount:
                metadata = Msg.Metadata(
                    ...
                    timestamp=int(tokens[7]) / 1_000_000_000.0,
                    stream=tokens[2],
                    consumer=tokens[3],
                )
            else:
                ...
                metadata = Msg.Metadata(
                    ...
                    timestamp=int(tokens[Msg.Ack.Timestamp]) / 1_000_000_000.0,
